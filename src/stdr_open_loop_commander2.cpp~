...
int main(int argc, char **argv) {
    ...
    double clockwise_yaw_rate = -0.5; //0.5 rad/sec yaw rate command
    double time_3_sec = 3.0; // should move 3 meters or 1.5 rad in 3 seconds
    double time_3_half_sec = 3.5;
    double time_1_sec = 1.0;
    double time_1_half_sec = 1.5;
    double time_2_sec = 2.1;
    ...
    twist_cmd.angular.z=clockwise_yaw_rate; //and start spinning in place
    timer=0.0; //reset the timer
    while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
    ...
    twist_cmd.angular.z=clockwise_yaw_rate; //and start spinning in place
    timer=0.0; //reset the timer
    while(timer<time_1_half_sec) { //twist about 45 degrees
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    twist_cmd.linear.x=speed; //and move forward again
    timer=0.0; //reset the timer
    while(timer<time_3_sec) { //move forward a little bit
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          } 
    ...
    twist_cmd.angular.z=clockwise_yaw_rate; //and start spinning in place
    timer=0.0; //reset the timer
    while(timer<time_1_half_sec) { //twist about 45 degrees again
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    twist_cmd.angular.z=0.0; //and stop spinning in place
    twist_cmd.linear.x=speed; //and move forward again toward top corner of the maze
    timer=0.0; //reset the timer
    while(timer<time_5_sec) { 
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
